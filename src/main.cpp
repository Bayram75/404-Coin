...
unsigned int nStakeMinAge = 186 * 60 * 60; // 7.77 day
...
const string strMessageMagic = "404 Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nRewardCoinYear = 404 * CENT;

    int64_t nSubsidy = 0 * COIN;

    if (pindexBest->nHeight < 30000)
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear * 2; // 808 %
      return nSubsidy + nFees;
    }  
            
    else if (pindexBest->nHeight < 350000)
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear * 1; // 404 %
      return nSubsidy + nFees;
    }  
...
// 404: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "404", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
